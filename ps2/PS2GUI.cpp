...
void DisplayIntroBox(const char *StrMsg)
{
   ...
   while ( true ) {
      ...
      gp_gouradrect(&thegp, (31+32-20), 70,GS_SET_RGBA(0x00, 0x00, 0x40, 0x80), 
         (20+32+255-31), (142+64),  GS_SET_RGBA(0x40,0x40, 0x80, 0x80), 3);
      
      textCpixel(0, 320, 80, GS_SET_RGBA(255, 255, 255, 255), 0, 0, 4, StrMsg);
      textCpixel(0, 320, 80+18+8, GS_SET_RGBA(255, 255, 255, 255), 0, 0, 4, 
         "InfoGB by 'dlanor' & 'KarasQ' rev.6a");
...
void ConfigButtons() {
   ...
   while ( msg[i] ) {
      gp_frect(&thegp, 0,0, 9, 240, Z_SCROLL_M, GS_SET_RGBA(0, 0, 0, 0x80));
...
void MenuOptions() {
   ...
   do
   {
      ...
      TextOut(32+5, 70+75, "Boot ELF:             Run", Z_SCROLL);
      ...
      switch ( Action ) {
         ...
         case PAD_CROSS:
            switch ( Entry ) {
               ...
               case 7: g_Sound ^= 1; CNF_edited |= 1; break;
               case 8: 
                  if ( ++g_Stretch > 2) { 
                     g_Stretch = 0; 
                  } 
...
void MenuBrowser() {
   ...
   while ( true ) {
      ...
      // Clear screen
      gp_frect(&thegp, 0,0, 9, 240, Z_SCROLL_M, GS_SET_RGBA(0, 0, 0, 128));
...
void Load_CNF(const char *CNF_path_p)
{
   ...
   for ( var_cnt = 0; get_CNF_string(&CNF_p, &name, &value); var_cnt++ ) { 
      ...
      else if(!strcmp((char *)name,"sound")) g_Sound = atoi((char *)value);
