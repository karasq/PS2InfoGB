...
void PS2_InitGS(void)
{
   ...
   if ( pal_ntsc() == 3 ) {
      ...
      g_SndSampler = 48000 / 50;
   } else {
      g_DispX = 80;
      g_DispY = 7;
      GS_InitGraph(GS_NTSC, GS_NONINTERLACE);
      GS_SetDispMode(g_DispX, g_DispY, SCREEN_W, SCREEN_H);
      g_SndSampler = 48000 / 60;
   }
...
void PS2_Init(void)
{
   ...
   // Sound init
   g_SndBuff[0] = (short int *)malloc(sizeof(short int)*g_SndSampler);
   g_SndBuff[1] = (short int *)malloc(sizeof(short int)*g_SndSampler);
  
   for ( i = 0; i < g_SndSampler; i++ ) 
      g_SndBuff[0][i] = g_SndBuff[1][i] = 0;
